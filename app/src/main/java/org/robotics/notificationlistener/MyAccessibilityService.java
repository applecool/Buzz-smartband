    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        if (eventType == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
            ...
            if (parcelable instanceof Notification) {
                ...
                Notification note = (Notification) parcelable;
                Bundle extras = note.extras;
                CharSequence messageText = extras.getCharSequence(Notification.EXTRA_TEXT);
                ...
                if (messages.size() > 0) {
                    ...
                    try {
                        Intent mIntent = new Intent(Constants.ACTION_CATCH_NOTIFICATION);
                        mIntent.putExtra(Constants.EXTRA_PACKAGE, sourcePackageName);
                        mIntent.putExtra(Constants.EXTRA_MESSAGE, messageText.toString());
                        MyAccessibilityService.this.getApplicationContext().sendBroadcast(mIntent);
                    } catch (Exception e) {
                        Log.e(TAG, e.toString());
                    }
